...
describe('authServerUrl', () => {
  // Pinned to the apex to match issuerFor() in 021-app; RFC 8414 compares the issuer byte for
  // byte against the identifier the client built the discovery URL from.
  test('stays on the apex even though the API base is www', () => {
    delete process.env.PUBLIC_APP_URL;
    expect(authServerUrl()).toBe('https://anomalia.so');
    expect(authServerUrl()).not.toBe(appUrl());
  });
